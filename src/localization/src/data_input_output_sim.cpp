...
// dependencies: 'vision/line.h','vision/corner.h','vision/goal.h','vision/opponents.h' header files for cv objects
...
//    tf::Stamped<tf::Pose> previous_opponent_field_pose;
...
//        previous_opponent_field_pose = opponent_field_pose;
    ...
    void visionLinesLandmarksCallback(const vision::LinesLandmarksConstPtr& lines_landmarks_msg) // need to change
    {
        ...
        BOOST_FOREACH(const vision::Landmark& temp_landmark, lines_landmarks_msg->landmarks)
        {
            ...
            temp_obj.pose.x = temp_landmark.pose.x;
            temp_obj.pose.y = temp_landmark.pose.y;
    ...
    void visionGoalCallback(const vision::GoalpostConstPtr& goal_msg)
    {
        if (goal_msg->goalpost_detected)
        {
            ...
            case 2: //add both posts and the center
                ...
                temp_obj.pose.x = (goal_msg->goalpost_left_range * cos(fabs(goal_msg->goalpost_left_bearing))
                                   + goal_msg->goalpost_right_range * cos(fabs(goal_msg->goalpost_right_bearing)))/2;
                temp_obj.pose.y = (goal_msg->goalpost_left_range * sin(goal_msg->goalpost_left_bearing)
                                   + goal_msg->goalpost_right_range * sin(goal_msg->goalpost_right_bearing))/2;
                msg_world_objects.objects.push_back(temp_obj);
                break;
            default:
    ...
    void visionOpponentCallback(const vision::OpponentsConstPtr& oppoents_msg)
       {
    }

//    // Callback function for opponent and other obstacles detected

//    void visionObstacleCallback(const vision::ObstaclesConstPtr& obstacle_msg)
//    {
...
//            msg_output_data.bOpponentWasSeen = 0;
//        }
//        if (obstacle_msg->obstacles_detected)
//        {

//            msg_output_data.bObstacleWasSeen=1;
//            msg_output_data.iObstacleNumber = obstacle_msg->obstacle_number;
...
//            point_tmp.x = 0;
//            point_tmp.y = 0;
//            // for all obstacles detected until the max number
//            for (int i=0; i< obstacle_msg->obstacle_number; i++)
//            {

//                // Data on image not given for now
//                msg_output_data.obstacleLeftEndInImage[i] = point_tmp;
//                msg_output_data.obstacleRightEndInImage[i] = point_tmp;

//                msg_output_data.obstacleDistance[i] = obstacle_msg->obstacle_range[i];
//                msg_output_data.obstacleAngle[i] = obstacle_msg->obstacle_bearing[i];

//                point_tmp.x = obstacle_msg->obstacle_range[i] * cos(fabs(obstacle_msg->obstacle_bearing[i]));
//                point_tmp.y = obstacle_msg->obstacle_range[i] * sin(obstacle_msg->obstacle_bearing[i]);
...
//                // Always add the obstacles if seen, but use the previous pose if the confidence isn't high
//                point_tmp.x = obstacle_field_pose.getOrigin().x();
//                point_tmp.y = obstacle_field_pose.getOrigin().y();
//                msg_output_data.obstacleCenterOnField[i] = point_tmp;
...
//                // Radius 0.5 for now
//                msg_output_data.obstacleRadiusOnField[i] = 0.5;
...
//    }
    ...
    void localizationMeanPoseCallback(const localization::MeanPoseConfStampedConstPtr& mean_pose_msg)
    {
        ...
        try{
            ...
            //ROS_ERROR("localization io: Couldn't find the '/robot_head' to '/robot_base' transform. %s",ex.what());
        }
