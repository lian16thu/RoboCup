    ...
    void visionGoalCallback(const vision::GoalpostConstPtr& goal_msg)
    {
        if (goal_msg->goalpost_detected)
        {
            ...
            //     ROS_ERROR("localization io: right goalpost seen at [%lf,%lf], range:%f, bearing:%f",temp_obj.pose.x,temp_obj.pose.y,goal_msg->goalpost_right_range,goal_msg->goalpost_right_bearing);
                ...
                temp_obj.pose.x = (goal_msg->goalpost_center_range * cos(fabs(goal_msg->goalpost_center_bearing));
                temp_obj.pose.y = (goal_msg->goalpost_center_range * sin(goal_msg->goalpost_center_bearing);
                msg_world_objects.goalposts.push_back(temp_obj);
             //   break;
            // default:
...
//    }
    ...
    void localizationMeanPoseCallback(const localization::MeanPoseConfStampedConstPtr& mean_pose_msg)
    {
        ...
        msg_output_data.robotHeadPose.theta = mean_pose_msg->robotPose.theta;
        ...
        msg_output_data.robotPose.x = msg_output_data.robotHeadPose.x + head_to_base.getOrigin().x();
        msg_output_data.robotPose.y = msg_output_data.robotHeadPose.y + head_to_base.getOrigin().y();
        msg_output_data.robotPose.theta = msg_output_data.robotHeadPose.theta + tf::getYaw(head_to_base.getRotation());
