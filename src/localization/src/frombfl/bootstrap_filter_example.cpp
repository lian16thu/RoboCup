...
int main(int argc, char** argv)
{
  ...
  Gaussian prior_cont(prior_Mu,prior_Cov);
   ...
   #define NUM_SAMPLES  2000 //number of samples

   //sampling from the previously created continuous prior distribution and store in vector
   vector<Sample<ColumnVector> > prior_samples(NUM_SAMPLES);
   MCPdf<ColumnVector> prior_discr(NUM_SAMPLES,3);
   //using samples create a discrete prior distribution, a monte carlo pdf
