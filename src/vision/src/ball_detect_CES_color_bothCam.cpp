...
//  Mat image_raw_Cam2;
  ...
  vision::Ball ball_msg_Cam1;
  vision::Ball ball_msg_Cam2;
...
public:
   BallDetector():it(nh)
   {
     ...
     image_sub_2 = it.subscribe("/camera2/rgb/image_rect_color", 1, &BallDetector::Detect2D_Callback_camera2, this);
...
//     image_raw_Cam2 = Mat::zeros(Height, Width, CV_8UC3);
    ...
    //Initialize ball msg
     ball_msg_Cam1.header.stamp = ros::Time::now();
     ball_msg_Cam1.ball_detected = false;
     ball_msg_Cam1.header.frame_id = reference_frame_Cam1;
     ball_msg_Cam1.ball_range = 0.0;
     ball_msg_Cam1.ball_bearing = 0.0;
     ball_msg_Cam1.ball_x = 0;
     ball_msg_Cam1.ball_y = 0;

     ball_msg_Cam2.header.stamp = ros::Time::now();
     ball_msg_Cam2.ball_detected = false;
     ball_msg_Cam2.header.frame_id = reference_frame_Cam2;
     ball_msg_Cam2.ball_range = 0.0;
     ball_msg_Cam2.ball_bearing = 0.0;
     ball_msg_Cam2.ball_x = 0;
     ball_msg_Cam2.ball_y = 0;
...
void BallDetector::Detect2D_Callback_camera1(const sensor_msgs::ImageConstPtr& Image_msg)
{
        ...
        startWindowThread();
        namedWindow("BallTracking2D_Cam1:raw");
        imshow("BallTracking2D_Cam1:raw", image_raw_last_frame_Cam1);
        waitKey(5);

        //--------------------------------Start Section I  find field---------------------------------//
        GaussianBlur(image_raw_last_frame_Cam1, image_raw_last_frame_Cam1, Size(5,5), 3, 3);
        cvtColor(image_raw_last_frame_Cam1, image_hsv_Cam1, COLOR_BGR2HSV);
        inRange(image_hsv_Cam1, Scalar(hmin, smin, vmin), Scalar(hmax, smax, vmax), image_hue_Cam1);

        //ball image
        imshow("BallTracking2D_Cam1:image_orange", image_hue_Cam1);
        waitKey(3);

 //       adaptiveThreshold(image_hue, image_hue, 255, CV_ADAPTIVE_THRESH_GAUSSIAN_C, CV_THRESH_BINARY, 9, 0);


        morphologyEx(image_hue_Cam1, image_hue_Cam1, MORPH_CLOSE, SE5x5);
        morphologyEx(image_hue_Cam1, image_hue_Cam1, MORPH_CLOSE, SE3x3);

        morphologyEx(image_hue_Cam1, image_hue_Cam1, MORPH_OPEN, SE3x3);
        erode(image_hue_Cam1, image_hue_Cam1, SE5x5);
        erode(image_hue_Cam1, image_hue_Cam1, SE5x5);

        Mat labels, stats, centroids;
        float radius_width, radius_height;
        int i, nccomps = connectedComponentsWithStats(image_hue_Cam1, labels, stats, centroids, 8);
...
//circle(image_raw_last_frame, Point2f(centroids.at<double>(i,0), centroids.at<double>(i,1)), cvRound(radius), Scalar(255,0,0), 2, LINE_AA);
                ...
                //draw object location on screen
                    ...
                    {
                        circle(image_raw_last_frame_Cam1, Point2f(centroids.at<double>(i,0), centroids.at<double>(i,1)), cvRound(radius_height), Scalar(0,0,255), 2, LINE_AA);
                        circle(image_raw_last_frame_Cam1, Point2f(centroids.at<double>(i,0), centroids.at<double>(i,1)), 2, Scalar(0,255,0), 2, LINE_AA);
                    }
                    else if(stats.at<int>(i, CC_STAT_TOP)==0)
                    {
                        circle(image_raw_last_frame_Cam1, Point2f(centroids.at<double>(i,0), centroids.at<double>(i,1)), cvRound(radius_width), Scalar(0,0,255), 2, LINE_AA);
                        circle(image_raw_last_frame_Cam1, Point2f(centroids.at<double>(i,0), centroids.at<double>(i,1)), 2, Scalar(0,255,0), 2, LINE_AA);

                    }
                    else if(stats.at<int>(i, CC_STAT_TOP) <= 250)
                    {
                        ROS_INFO("Ball is out of range, please put it closer");
                    }
                    else
                    {
                        circle(image_raw_last_frame_Cam1, Point2f(centroids.at<double>(i,0), centroids.at<double>(i,1)), cvRound(MAX(radius_width,radius_height)), Scalar(0,0,255), 2, LINE_AA);
                        circle(image_raw_last_frame_Cam1, Point2f(centroids.at<double>(i,0), centroids.at<double>(i,1)), 2, Scalar(0,255,0), 2, LINE_AA);
 ...
 //-------------------------
        imshow("BallTracking2D:Detected by Cam1", image_raw_last_frame_Cam1);
        waitKey(5);
...
//        }
        ...
        if(found)
        {
            ...
            ROS_ERROR("Ball detected by Cam1, center is %f, %f", ball_position.x, ball_position.y);
           ...
           cout << "Cam1: 3D location of ball is :" << coord_3d.at<double>(0)-27 << "," << coord_3d.at<double>(1)- ball_radius <<endl;

           ball_msg_Cam1.header.stamp = ros::Time::now();
           ball_msg_Cam1.ball_detected = true;

           ball_msg_Cam1.ball_x = (coord_3d.at<double>(0)-27)/100;
           ball_msg_Cam1.ball_y = (coord_3d.at<double>(1)- ball_radius)/100;
           ball_msg_pub.publish(ball_msg_Cam1);
...
void BallDetector::Detect2D_Callback_camera2(const sensor_msgs::ImageConstPtr& Image_msg)
{
        ...
        startWindowThread();
        namedWindow("BallTracking2D_Cam2:raw");
        imshow("BallTracking2D_Cam2:raw", image_raw_last_frame_Cam2);
        waitKey(5);
...
//                waitKey(5);
        ...
        //--------------------------------Start Section I  find field---------------------------------//
        GaussianBlur(image_raw_last_frame_Cam2, image_raw_last_frame_Cam2, Size(5,5), 3, 3);
        cvtColor(image_raw_last_frame_Cam2, image_hsv_Cam2, COLOR_BGR2HSV);
        inRange(image_hsv_Cam2, Scalar(hmin, smin, vmin), Scalar(hmax, smax, vmax), image_hue_Cam2);

        //ball image
        imshow("BallTracking2D_Cam2:image_orange", image_hue_Cam2);
        waitKey(3);

 //       adaptiveThreshold(image_hue, image_hue, 255, CV_ADAPTIVE_THRESH_GAUSSIAN_C, CV_THRESH_BINARY, 9, 0);


        morphologyEx(image_hue_Cam2, image_hue_Cam2, MORPH_CLOSE, SE5x5);
        morphologyEx(image_hue_Cam2, image_hue_Cam2, MORPH_CLOSE, SE3x3);

        morphologyEx(image_hue_Cam2, image_hue_Cam2, MORPH_OPEN, SE3x3);
        erode(image_hue_Cam2, image_hue_Cam2, SE5x5);
        erode(image_hue_Cam2, image_hue_Cam2, SE5x5);

//-------------------------
        Mat labels, stats, centroids;
        float radius_width, radius_height;
        int i, nccomps = connectedComponentsWithStats(image_hue_Cam2, labels, stats, centroids, 8);
...
//circle(image_raw_last_frame, Point2f(centroids.at<double>(i,0), centroids.at<double>(i,1)), cvRound(radius), Scalar(255,0,0), 2, LINE_AA);
                ...
                //draw object location on screen
                    ...
                    if(stats.at<int>(i, CC_STAT_LEFT)==0)
                    {
                        circle(image_raw_last_frame_Cam2, Point2f(centroids.at<double>(i,0), centroids.at<double>(i,1)), cvRound(radius_height), Scalar(0,0,255), 2, LINE_AA);
                        circle(image_raw_last_frame_Cam2, Point2f(centroids.at<double>(i,0), centroids.at<double>(i,1)), 2, Scalar(0,255,0), 2, LINE_AA);
                    }
                    else if(stats.at<int>(i, CC_STAT_TOP)==0)
                    {
                        circle(image_raw_last_frame_Cam2, Point2f(centroids.at<double>(i,0), centroids.at<double>(i,1)), cvRound(radius_width), Scalar(0,0,255), 2, LINE_AA);
                        circle(image_raw_last_frame_Cam2, Point2f(centroids.at<double>(i,0), centroids.at<double>(i,1)), 2, Scalar(0,255,0), 2, LINE_AA);
                    ...
                    else if(stats.at<int>(i, CC_STAT_TOP) > 311)
                    {
                        ...
                        circle(image_raw_last_frame_Cam2, Point2f(centroids.at<double>(i,0), stats.at<int>(i, CC_STAT_TOP) + 85), 2, Scalar(0,255,0), 2, LINE_AA);
                        ROS_ERROR("Ball occuluded by Cam2, y is %d ", stats.at<int>(i, CC_STAT_TOP) + 85);

                    }
                    else
                    {
                        circle(image_raw_last_frame_Cam2, Point2f(centroids.at<double>(i,0), centroids.at<double>(i,1)), cvRound(MAX(radius_width,radius_height)), Scalar(0,0,255), 2, LINE_AA);
                        circle(image_raw_last_frame_Cam2, Point2f(centroids.at<double>(i,0), centroids.at<double>(i,1)), 2, Scalar(0,255,0), 2, LINE_AA);
 ...
 //-------------------------
        imshow("BallTracking2D:Detected by Cam2", image_raw_last_frame_Cam2);
        waitKey(5);
...
//        }
        ...
        if(found)
        {
            ...
            ROS_ERROR("Ball detected by Cam2, center is %f, %f", ball_position.x, ball_position.y);
           ...
           cout << "Cam 2: 3D location of ball is :" << coord_3d.at<double>(0)-27 << "," << coord_3d.at<double>(1)- ball_radius <<endl;

           ball_msg_Cam2.header.stamp = ros::Time::now();
           ball_msg_Cam2.ball_detected = true;

           ball_msg_Cam2.ball_x = (coord_3d.at<double>(0)-27)/100;
           ball_msg_Cam2.ball_y = (coord_3d.at<double>(1)- ball_radius)/100;
           ball_msg_pub.publish(ball_msg_Cam2);
