...
public:
        ...
        : it_(pcl_nh)
    {
        ...
        sub_imu = pcl_nh.subscribe ("/imu/data", 1, &PointCloudProcess::imuCb,this);
                         ...
                         stereo_process::DepthRequest::Response &res)
    {
        ...
        if (cloud->isOrganized())
        {
            while (!((point_range>0)&&(point_range<100)))
            {
                ...
                pcl_pt.x = cloud->at(image_x,image_y).x;
                pcl_pt.y = cloud->at(image_x,image_y).y;
                pcl_pt.z = cloud->at(image_x,image_y).z;
                //ROS_INFO("After use of x,y");
                point_depth = (pcl_pt.z-pcl_pt.y*sin(pitch_dif) ) * cos(pitch_dif);
            ...
            //ROS_ERROR("Point response:x:%f, y:%f, z:%f, range:%f req u:%d, req v:%d",pcl_pt.x,
            //          pcl_pt.y,pcl_pt.z,point_range,image_x-1,image_y-1);
        ...
        //ROS_ERROR("Object depth response:%f, x:%f, y:%f, req u:%d, req v:%d",pt_cv.z,pt_cv.x,pt_cv.y,req.u,req.v);

        //res.depth = pt_cv.z;
        res.depth =point_range;
        //ROS_INFO("res.depth %f",res.depth);
        return true;
    }
    ...
    {\
        ...
        if ((point_hsv.h>=80 && point_hsv.h<=160)
                && (point_hsv.s>=0 && point_hsv.s<=1.00)
                && (point_hsv.v>=0.00 && point_hsv.v<=1.00)
                )
            return true;
        else
        {   //if (point_hsv.h>250)
    ...
    void cloudProcess()
    {
        ...
        sensor_msgs::PointCloud2Ptr cloud_2_process;
        ...
        // Create a local copy of the pcl to be processed
        cloud_2_process = current_pcl;
        // Convert the sensor_msgs/PointCloud2 data to pcl/PointCloud
        pcl::fromROSMsg(*cloud_2_process,*cloud);
        ...
        pitch_dif = -(imu_roll-init_imu_roll);
        pitch_dif += 0.436; // 25 degrees offset for now
        yaw_dif = -(imu_pitch-init_imu_pitch);
        ...
        field_image.header.stamp = cloud_2_process->header.stamp;
        ...
        for (std::vector<pcl::PointIndices>::const_iterator it = cluster_indices.begin (); it != cluster_indices.end (); ++it)
        {
            ...
            cluster_y_zmin = cloud_above_field->points[*(it->indices.begin ())].y;
            ...
            for (std::vector<int>::const_iterator pit = it->indices.begin (); pit != it->indices.end (); ++pit)
            {
                ...
                if (cloud_above_field->points[*pit].z<cluster_zmin)
                {   cluster_zmin = cloud_above_field->points[*pit].z;
                    cluster_y_zmin = cloud_above_field->points[*pit].y;
                }
            }
            ...
            cluster_zmin = (cluster_zmin-cluster_ymax*sin(pitch_dif) ) * cos(pitch_dif);
            //cluster_zmin = (cluster_zmin-cluster_y_zmin*sin(pitch_dif) ) * cos(pitch_dif);

            cluster_height = (cluster_ymax - cluster_ymin) * cos(pitch_dif);

            cluster_height_max = CAMERA_HEIGHT -cluster_ymin * cos(pitch_dif) - cluster_zmax * sin(pitch_dif); // need to check this... it has to do with range?
            ...
            if (pitch_dif*180/M_PI>60)
            {
                cluster_range = 2*cluster_range - 0.56; // after studying the real offset
                if (cluster_range<0)
                    cluster_range = 0;
            }

            if (cluster_xmean!=0)
                cluster_bearing = atan2(cluster_xmean,cluster_zmin); //range [0,360]
            else
                cluster_bearing = 0.0;

            //ROS_INFO("Camera pitch difference:%f, cluster height:%f, range:%f",pitch_dif,cluster_height,cluster_range);
            //ROS_INFO("PointCloud representing the Cluster: %d data points, x_min:%f, x_max:%f, y_min:%f, y_max:%f, z_min:%f, z_max:%f, y_zmin:%f",
              //       cloud_cluster->points.size(),cluster_xmin,cluster_xmax,cluster_ymin,cluster_ymax,cluster_zmin,cluster_zmax,cluster_y_zmin);

            // Classification for ball on the ground

            if ((cluster_height<=0.35)
                    && (cluster_height>0.08)
                    && (cluster_height_max<=0.50)
                    //&& (cluster_height_max<=0.50)
                    && (cluster_width<=0.45)
                    && (!ball_detected)
                    )
            {
                ROS_INFO("Ball found, cluster ball height:%f, width:%f, max_height:%f, range:%f, bearing:%f, size:%d",
                         cluster_height,cluster_width,cluster_height_max,cluster_range,cluster_bearing* 180 / M_PI,cloud_cluster->width);
                ...
                stereo_process::ObjectOnImage ball_image;
                //ball_image.header.stamp = ros::Time::now();
                ball_image.header.stamp = cloud_2_process->header.stamp;
                ball_image.object_type = 1;
                ball_image = convert3dTo2d(*cloud_cluster,ball_image);

                ball_image.object_range = cluster_range;
                ball_image.object_bearing = cluster_bearing;
                ball_image.head_pitch = pitch_dif;

                pub_ball.publish(ball_image);

//                                // Publish ball info as vision
//                                stereo_process::Ball ball_msg;
//                                ball_msg.header.stamp = cloud_2_process->header.stamp;
//                                ball_msg.ball_detected = true;
//                                ball_msg.ball_range = cluster_range;
//                                ball_msg.ball_bearing = cluster_bearing/180 *M_PI;
...
//                                pub_vision_ball.publish(ball_msg);
            ...
            else if ((cluster_height>0.35)
                     && (cluster_height_max>0.08)
                     // && (cluster_width<=0.80)
                     )
            {
                ...
                if (j==0)
                {
                    ...
                    ROS_INFO("Obstacle1 at range:%f, bearing:%f, size:%d, height:%f, width:%f, height_max:%f",cluster_range,cluster_bearing* 180 / M_PI,cloud_cluster->width, cluster_height, cluster_width, cluster_height_max);
                ...
                if (j==1)
                {
                    ...
                    ROS_INFO("Obstacle2 at range:%f, bearing:%f, size:%d, height:%f, width:%f, height_max:%f",cluster_range,cluster_bearing* 180 / M_PI,cloud_cluster->width, cluster_height, cluster_width, cluster_height_max);
                }
                if (j==2)
                {
                    pcl::toROSMsg(*cloud_cluster,output_obstacle3);
                    pub_cluster_obstacle_3.publish(output_obstacle3);
                    ROS_INFO("Obstacle3 at range:%f, bearing:%f, size:%d, height:%f, width:%f, height_max:%f",cluster_range,cluster_bearing* 180 / M_PI,cloud_cluster->width, cluster_height, cluster_width, cluster_height_max);
                }
                if (j==3)
                {
                    pcl::toROSMsg(*cloud_cluster,output_obstacle4);
                    pub_cluster_obstacle_4.publish(output_obstacle4);
                    ROS_INFO("Obstacle4 at range:%f, bearing:%f, size:%d, height:%f, width:%f, height_max:%f",cluster_range,cluster_bearing* 180 / M_PI,cloud_cluster->width, cluster_height, cluster_width, cluster_height_max);
                }
                if (j==4)
                {
                    pcl::toROSMsg(*cloud_cluster,output_obstacle5);
                    pub_cluster_obstacle_5.publish(output_obstacle5);
                    ROS_INFO("Obstacle5 at range:%f, bearing:%f, size:%d, height:%f, width:%f, height_max:%f",cluster_range,cluster_bearing* 180 / M_PI,cloud_cluster->width, cluster_height, cluster_width, cluster_height_max);
                }
                ...
                object_image.header.stamp = cloud_2_process->header.stamp;
                object_image.object_type = 3;
                object_image = convert3dTo2d(*cloud_cluster,object_image);
                object_image.object_range = cluster_range;
                object_image.object_bearing = cluster_bearing;
                object_image.head_pitch = pitch_dif;
                object_image.object_width = cluster_width;
                //object_image.object_height = cluster_height;
            ...
            {//ROS_INFO("Unclassified cluster range:%f, bearing:%f, size:%d, height:%f, width:%f, height_max:%f",
               //       cluster_range,cluster_bearing* 180 / M_PI,cloud_cluster->width, cluster_height, cluster_width, cluster_height_max);
            }
