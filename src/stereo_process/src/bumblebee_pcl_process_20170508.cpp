...
public:
    ...
    void cloudCb (const sensor_msgs::PointCloud2Ptr& input)
    {
        ...
        pass.setInputCloud (cloud_filtered);
        pass.setFilterFieldName("y");
        pass.setFilterLimits (-11*sin(pitch_dif), +1.3); // to avoid missing points higher
        pass.filter (*cloud_filtered);
        ...
        double z_min = 0.08, z_max = 1.3; // we want the points above the plane, but no ground. no further than 1.3m from the surface
        ...
        for (std::vector<pcl::PointIndices>::const_iterator it = cluster_indices.begin (); it != cluster_indices.end (); ++it)
        {
            ...
            cluster_height_max = 1.3 -cluster_ymin * cos(pitch_dif) - cluster_zmin * sin(pitch_dif);
