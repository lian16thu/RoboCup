...
class PointCloudProcess
{
    ...
    std::vector<double> oppo_temp_bearing;
    std::vector<double> oppo_temp_range;
...
public:
        ...
        : it_(pcl_nh)
    {
        ...
        oppo_msg_pub = pcl_nh.advertise<vision::Opponents>("/vision/opponents", 1);
        ...
        //Initialize opponent msg
        oppo_msg.header.stamp = ros::Time::now();
        oppo_msg.opponent_detected = false;
        oppo_msg.opponent_number = 0;
    ...
    void detectionCb(const darknet_ros_msgs::BoundingBoxes::ConstPtr& msg)
    {
        ...
        oppo_temp_bearing.clear();
        oppo_temp_range.clear();
        oppo_number_counter = 0;
...
//                                pcl_pt.z = cloud->at(BBox_center.x,BBox_center.y).x;
                                ...
                                //ROS_WARN("depth is %f", pcl_pt.z);

//                                if (((pcl_pt.z > 0.9) && (pcl_pt.z < 2.0) )
//                                {
                                    found_ball = true;
...
//                                {
//                                    found_ball = false;
//                                    ROS_WARN("Found part of body, not real ball, depth is %f", pcl_pt.z);
//                                    break;
...
//                        }
...

                {
                    ...
                    ROS_ERROR("f2w_x:%f, f2w_y:%f, head_pitch:%f, head_yaw:%f",front_to_waist_x,front_to_waist_y,head_pitch,temp_head_yaw);
                    double front_to_camera_center_x = front_to_waist_x + 0.16 * sin(head_pitch) * cos(temp_head_yaw);
                    double camera_center_to_left_x = - D_CAM_LEFT *sin(temp_head_yaw)
                            + landmarks_msg.landmark_range[landmark_number_counter] * cos(temp_head_yaw+landmarks_msg.landmark_bearing[landmark_number_counter]);
                    double front_to_camera_center_y = front_to_waist_y + 0.16 * sin(head_pitch) * sin(temp_head_yaw);
                    double camera_center_to_left_y = + D_CAM_LEFT *cos(temp_head_yaw)
                            + landmarks_msg.landmark_range[landmark_number_counter] * sin(temp_head_yaw+landmarks_msg.landmark_bearing[landmark_number_counter]);

                    double front_to_goal_x = front_to_camera_center_x + camera_center_to_left_x;
                    double front_to_goal_y = front_to_camera_center_y + camera_center_to_left_y;

                    landmarks_msg.landmark_range[landmark_number_counter] = sqrt(pow(front_to_goal_x,2)+pow(front_to_goal_y,2));
                    landmarks_msg.landmark_bearing[landmark_number_counter] = atan2(front_to_goal_y,front_to_goal_x);
                ...
                if(BBox.Class == "Goalpost")
                {
                    ...
                    ROS_ERROR("f2w_x:%f, f2w_y:%f, head_pitch:%f, head_yaw:%f",front_to_waist_x,front_to_waist_y,head_pitch,temp_head_yaw);
                    double front_to_camera_center_x = front_to_waist_x + 0.16 * sin(head_pitch) * cos(temp_head_yaw);
                    double camera_center_to_left_x = - D_CAM_LEFT *sin(temp_head_yaw) + point_range * cos(temp_head_yaw+temp_bearing);
                    double front_to_camera_center_y = front_to_waist_y + 0.16 * sin(head_pitch) * sin(temp_head_yaw);
                    double camera_center_to_left_y = + D_CAM_LEFT *cos(temp_head_yaw) + point_range * sin(temp_head_yaw+temp_bearing);

                    double front_to_goal_x = front_to_camera_center_x + camera_center_to_left_x;
                    double front_to_goal_y = front_to_camera_center_y + camera_center_to_left_y;

                    point_range = sqrt(pow(front_to_goal_x,2)+pow(front_to_goal_y,2));
                    temp_bearing = atan2(front_to_goal_y,front_to_goal_x);
                ...
                //Opponent and Person detection as obstacles
                if(BBox.Class == "Opponent")
                {
                    found_oppo = true;
                    double oppo_bearing;
                    double oppo_range;
                    ...
                    oppo_msg.opponent_range[oppo_number_counter] = point_range;

                    if(BBox_center.x >= 640)
                    {
                        //bearing direction is positive
                        oppo_msg.opponent_bearing[oppo_number_counter] = -atan((BBox_center.x - 640)/fx);
                    }
                    else
                    {
                        //bearing direction is negative
                        oppo_msg.opponent_bearing[oppo_number_counter] = atan((640 - BBox_center.x)/fx);

                    }

                    ROS_ERROR("f2w_x:%f, f2w_y:%f, head_pitch:%f, head_yaw:%f",front_to_waist_x,front_to_waist_y,head_pitch,temp_head_yaw);
                    double front_to_camera_center_x = front_to_waist_x + 0.16 * sin(head_pitch) * cos(temp_head_yaw);
                    double camera_center_to_left_x = - D_CAM_LEFT *sin(temp_head_yaw) + point_range * cos(temp_head_yaw+oppo_bearing);
                    double front_to_camera_center_y = front_to_waist_y + 0.16 * sin(head_pitch) * sin(temp_head_yaw);
                    double camera_center_to_left_y = + D_CAM_LEFT *cos(temp_head_yaw) + point_range * sin(temp_head_yaw+oppo_bearing);

                    double front_to_goal_x = front_to_camera_center_x + camera_center_to_left_x;
                    double front_to_goal_y = front_to_camera_center_y + camera_center_to_left_y;

                    oppo_msg.opponent_range[oppo_number_counter] = sqrt(pow(front_to_goal_x,2)+pow(front_to_goal_y,2));
                    oppo_msg.opponent_bearing[oppo_number_counter] = atan2(front_to_goal_y,front_to_goal_x);


                    ROS_ERROR("Opponent bearing is %f, range is %f, type:%d",
                              oppo_msg.opponent_bearing[oppo_number_counter]*180/PI,
                              oppo_msg.opponent_range[oppo_number_counter]);
            ...
            //publish opponent info to ROS
            if(found_oppo)
            {
                //publish landmarks message to ROS
                oppo_msg.header.stamp = msg->header.stamp;
                oppo_msg.opponent_detected = true;
                oppo_msg.opponent_number = oppo_number_counter;

                ROS_INFO("%d opponent found", oppo_number_counter);
