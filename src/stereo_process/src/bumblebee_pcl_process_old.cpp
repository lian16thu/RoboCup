...
public:
                         ...
                         stereo_process::DepthRequest::Response &res)
    {
        ...
        if (cloud->isOrganized())
        {
            ...
            ROS_ERROR("Point response:x:%f, y:%f, z:%f, req u:%d, req v:%d",pcl_pt.x,
                      pcl_pt.y,pcl_pt.z,image_x,image_y);
    ...
    {\
        ...
        if ((point_hsv.h>=94 && point_hsv.h<=150)
                && (point_hsv.s>=0.37 && point_hsv.s<=1.00)
                && (point_hsv.v>=0.08 && point_hsv.v<=0.67)
                )
            return true;
        else
        {   //if (point_hsv.h>250)
